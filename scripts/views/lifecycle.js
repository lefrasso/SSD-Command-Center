...
const ONBOARD_TASKS = ['Bootcamp: Delivery fundamentals', 'Bootcamp: Track deep-dive', 'Role guidance & shadowing', 'Tools & access provisioning', 'First supervised delivery'];
...
function openProfile(id) {
  ...
  const onboardDone = c.lifecycle === 'active' || c.lifecycle === 'offboarding' ? ONBOARD_TASKS.length : seed % (ONBOARD_TASKS.length + 1);
  const readiness = Math.round((onboardDone / ONBOARD_TASKS.length) * 100);
  ...
  const stageSection = c.lifecycle === 'offboarding'
    ...
    : `<div class="section-title">Onboarding tracker</div>${checklist(ONBOARD_TASKS, onboardDone)}
       ...
       <div class="muted mt8" style="font-size:12px">${readiness >= 80 ? 'Ready for unsupervised delivery.' : readiness >= 40 ? 'On track — complete remaining bootcamp items.' : 'Early in onboarding; assign a mentor.'}</div></div>`;

// Quality Checks — Quality Check forms + reporting, and Mock Deliveries.
import { store } from '../store.js';
import { pageHeader, kpiCard, aiChip, esc, badge, scoreColor, COLORS, clearCharts, bar, openDrawer } from '../components.js';
...
export function renderQuality(container) {
  const tabs = [['qc', 'Quality Checks'], ['mock', 'Mock Deliveries']]
    .filter(([key]) => key !== 'qc' || store.role !== 'partner-csa');
  if (!tabs.some(([key]) => key === tab)) tab = tabs[0][0];
  container.innerHTML = `
    ${pageHeader({ title: 'Quality Checks', description: 'Quality Check reviews and Mock Deliveries, scored against the Recommended Practices checklist.' })}
  ...
  if (tab === 'qc') renderQc(tc);
  else renderMock(tc);
}

// ---- Tab 1: Quality Checks (form + reporting) ----

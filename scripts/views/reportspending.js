...
import { computeT3W, T3W_WINDOW_DAYS, daysUntilDue, outreachCount, t3wReason, T3W_STEPS, t3wDraft, nextStepIndex, isStepDue } from '../t3w.js';
import { openEngagement, OUTREACH_DAYS } from './engagements.js';
...
export function renderReportsPending(container) {
  ...
  container.innerHTML = `
    ...
    <div class="muted mb8" style="font-size:12px">Engagements due within 3 weeks. The Partner CSA owns the Step 0–3 proactive dispatch cadence (confirm the kickoff meeting, follow up, escalate, then loop in the Resource Manager) to prevent the report becoming pending — send or automate it right here.</div>
    <div class="table-wrap"><table class="grid"><thead><tr><th>Customer</th><th>CSA</th><th>Family</th><th>Due in</th><th>Day 0</th><th>Day 1</th><th>Day 2</th><th>Day 3</th><th>Proactive status</th><th>Next step</th><th>Proactive action</th></tr></thead><tbody>
      ${inWindow.slice().sort((a, b) => daysUntilDue(a) - daysUntilDue(b)).map((e) => { const c = csaOf(e); const dd = daysUntilDue(e); const ps = proStatus(e); const canAct = (my && my.id === e.assignedTo) || can(store.role, 'edit:dispatch'); const remaining = OUTREACH_DAYS.filter((k) => !e.outreach[k]); const idx = nextStepIndex(e); const due = idx >= 0 && isStepDue(e); return `<tr>
        <td><strong>${esc(e.customer)}</strong></td>
        <td>${esc(c ? c.name : 'Unassigned')}</td>
        <td>${esc(e.track)}</td>
        <td>${dd}d</td>
  ...
  container.querySelectorAll('[data-rp-auto]').forEach((b) => b.addEventListener('click', () => {
    ...
    const drafts = {}; T3W_STEPS.forEach((s) => { if (!eng.outreach[s.key]) drafts[s.key] = t3wDraft(s, eng, d, csaName); });
    sendAllRemainingSteps(eng.id, drafts, 'Outreach Concierge');
  }));

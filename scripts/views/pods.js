...
import { TZ_MAP } from '../../data/generate.js';
      ...
      <div style="display:grid; grid-template-columns:repeat(auto-fill,minmax(220px,1fr)); gap:10px">
        ${pods.map((p) => {
          ...
          return `<div class="heat-cell" style="background:${utilColor(u)}">
            ...
            <div style="font-size:11px;opacity:.9;margin-top:2px">${esc(p.tz)} · ${csas.length} CSAs · ${esc(p.tzLead)}</div>
          </div>`;
        }).join('')}
      </div>
    </div>
    ...
    <div class="two-col">
      <div>
        ...
        <div class="table-wrap">
          <table class="grid">
            <thead><tr><th>Name</th><th>Vendor</th><th>POD</th><th>Tracks</th><th>Utilization</th><th>Tenure</th><th>Status</th></tr></thead>
            <tbody>
